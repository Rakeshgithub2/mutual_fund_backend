...
class WeeklyGraphJob {
    ...
    static async execute() {
        ...
        for (let i = 0; i < funds.length; i += batchSize) {
            ...
            try {
                const results = await graph_data_service_1.default.bulkAggregateGraphData(fundIds);
                succeeded += results.length;
                processed += batch.length;
                console.log(`[Weekly Graph Job] Progress: ${processed}/${funds.length}`);
            }
            catch (error) {
     ...
     */
    static async aggregateFund(fundId) {
        console.log(`[Weekly Graph Job] Aggregating fund ${fundId}...`);
        const results = await graph_data_service_1.default.bulkAggregateGraphData([fundId]);
        return {
            success: true,
            fundId,
    ...
    static async cleanup() {
        ...
        const result = await graph_data_service_1.default.cleanupStaleData();
        console.log(`[Weekly Graph Job] Deleted ${result.deletedCount} stale records`);
        return result;
    }
    /**
...
exports.WeeklyGraphJob = WeeklyGraphJob;
exports.default = WeeklyGraphJob;
