 ...
 */
async function generateGeminiResponse(query, context) {
    if (!genAI) {
 ...
 */
function findBestMatch(query, keywords) {
...
- High Beta in bull markets, Low Beta in uncertain times`;
    ...
    if (lowerQuery.includes('standard deviation') ||
...
💡 **Sweet Spot**: ₹2,000-8,000 Cr for equity funds`;
    ...
    // Exit Load
    if (lowerQuery.includes('exit load') ||
...
- Prevents concentration risk`;
    ...
    // Index Funds
    if (lowerQuery.includes('index fund') ||
 ...
 */
function getFollowUpQuestions(query) {
    const lowerQuery = query.toLowerCase();
    if (lowerQuery.includes('sip')) {
        return [
