...
router.get('/quick', async (req, res) => {
    try {
        ...
        try {
            await redis_1.redis.set(cacheKey, response, 300);
...
router.get('/count', async (req, res) => {
    try {
        ...
        // Cache for 1 hour
        await redis_1.redis.set(cacheKey, response, 3600);
        return res.json(response);
    }
    catch (error) {
 ...
 */
router.get('/batch/:page', async (req, res) => {
    try {
        ...
        try {
            await redis_1.redis.set(cacheKey, response, 600);
