...
router.get('/:userId/transactions', async (req, res) => {
    try {
        ...
        const history = await portfolioService.getTransactionHistory(userId, limit ? parseInt(limit) : 50);
        res.json({
            success: true,
            data: history,
        });
...
router.post('/:userId/transaction', async (req, res) => {
    try {
        ...
        if (!transaction.type ||
            ...
            return res.status(400).json({
                success: false,
                message: 'Missing required transaction fields: type, fundId, units, price',
            });
        }
