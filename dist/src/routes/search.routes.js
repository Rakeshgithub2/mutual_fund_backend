 ...
 */
router.get('/suggest', throttle, async (req, res) => {
    try {
        ...
        // Check cache
        const cached = await redis_1.redis.getSuggestions(query);
        if (cached) {
            return res.json({
                success: true,
                data: cached,
        ...
        const searchService = new search_service_1.SearchService(db);
        ...
        // Cache for 1 hour
        await redis_1.redis.cacheSuggestions(query, response);
        res.json({
            success: true,
            data: response,
            cached: false,
 ...
 * - limit: Max results (default: 20, max: 15000)
 *
 * Features:
 ...
 */
router.get('/funds', throttle, async (req, res) => {
    try {
        const { q, category, minAum, fuzzy = 'true', boost = 'true', limit = '20', } = req.query;
        if (!q || typeof q !== 'string') {
            return res.status(400).json({
                success: false,
                message: 'Query parameter "q" is required',
        ...
        const minAumNum = minAum ? parseFloat(minAum) : undefined;
        // Check cache
        ...
        const layerStats = results.reduce((acc, r) => {
        ...
        await redis_1.redis.set(cacheKey, response, 30 * 60);
        res.json({
            success: true,
            data: response,
            cached: false,
 ...
 */
router.get('/by-tags', throttle, async (req, res) => {
    try {
