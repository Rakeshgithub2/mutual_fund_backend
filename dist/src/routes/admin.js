    ...
    try {
        ...
        if (!process.env.NEWSDATA_API_KEY) {
            return res.json((0, response_1.formatResponse)({
                ...
                message: 'Using static news data. To fetch real news, configure NEWSDATA_API_KEY in .env file.',
                apiInfo: 'Get free API key from: https://newsdata.io/register',
            }, 'News API key not configured'));
        }
        const result = await newsService_1.newsService.ingestNews('business', [
            'mutual fund india',
            'mutual funds',
            'sip investment',
            'equity market india',
...
router.get('/refresh/status', async (req, res) => {
    try {
        const newsCount = await newsService_1.newsService.getRecentNews(1);
        return res.json((0, response_1.formatResponse)({
