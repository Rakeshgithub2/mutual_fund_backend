    ...
    try {
        ...
        if (!monthlyInvestment || !expectedReturn || !timePeriod) {
            return res.status(400).json({
                statusCode: 400,
                message: 'Missing required fields: monthlyInvestment, expectedReturn, timePeriod',
            });
        }
...
router.post('/lumpsum', async (req, res) => {
    try {
        const { investment, expectedReturn, timePeriod } = req.body;
        if (!investment || !expectedReturn || !timePeriod) {
            return res.status(400).json({
                statusCode: 400,
                message: 'Missing required fields: investment, expectedReturn, timePeriod',
            });
        }
...
router.post('/goal', async (req, res) => {
    try {
        const { targetAmount, timePeriod, expectedReturn, currentSavings } = req.body;
        if (!targetAmount || !timePeriod || !expectedReturn) {
            return res.status(400).json({
                statusCode: 400,
                message: 'Missing required fields: targetAmount, timePeriod, expectedReturn',
            });
        }
        ...
        return res.json((0, response_1.formatResponse)({
            ...
            breakdown: {
                ...
                returnsOnSIP: Math.round(target - currentValueAtGoal - monthlySIP * n),
            },
...
router.post('/step-up-sip', async (req, res) => {
    try {
        const { initialSIP, stepUpPercentage, expectedReturn, timePeriod } = req.body;
...
router.post('/retirement', async (req, res) => {
    try {
        const { currentAge, retirementAge, currentSavings, monthlyExpense, expectedReturn, inflationRate, } = req.body;
        ...
        const futureMonthlyExpense = monthlyExp * Math.pow(1 + inflation, yearsToRetirement);
