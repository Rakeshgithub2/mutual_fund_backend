...
const yahooFinance = new yahooFinanceService_1.YahooFinanceService();
...
router.get('/history/:symbol', async (req, res) => {
    try {
        ...
        const result = await yahooFinance.fetchHistoricalData(yahooSymbol, fromDate.toISOString(), toDate.toISOString());
        if (!result.success || !result.data) {
            console.warn(`⚠️ Yahoo Finance failed, using mock data for ${symbol}`);
            // Generate mock data as fallback
            const mockData = generateMockHistoricalData(symbol, fromDate, toDate, period);
            return res.json({
                success: true,
                data: mockData,
                source: 'mock',
...
function generateMockHistoricalData(symbol, fromDate, toDate, period) {
    ...
    for (let i = 0; i < dataPoints; i++) {
        ...
        if (period === '1D') {
            ...
            date.setDate(fromDate.getDate() + Math.floor((i / dataPoints) * daysDiff));
        }
