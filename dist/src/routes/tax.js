...
router.post('/capital-gains', async (req, res) => {
    try {
        const { investmentType, // EQUITY or DEBT
        purchasePrice, salePrice, units, purchaseDate, saleDate, isIndexed, // For LTCG in debt funds (bought before Apr 1, 2023)
         } = req.body;
        if (!investmentType ||
            !purchasePrice ||
            !salePrice ||
        ...
        const holdingPeriodDays = Math.floor((sale.getTime() - purchase.getTime()) / (1000 * 60 * 60 * 24));
        const holdingPeriodMonths = Math.floor(holdingPeriodDays / 30);
        let capitalGain = (parseFloat(salePrice) - parseFloat(purchasePrice)) * parseFloat(units);
        let taxRate = 0;
        let taxAmount = 0;
        let gainType = '';
        ...
        return res.json((0, response_1.formatResponse)({
            ...
            notes: investmentType === 'DEBT' && taxRate === 0
...
router.post('/sip-gains', async (req, res) => {
    try {
        ...
        if (!Array.isArray(installments) ||
            ...
            return res.status(400).json({
                statusCode: 400,
                message: 'Invalid input. Provide installments array with purchaseDate, units, and nav',
            });
        }
        const sale = new Date(saleDate);
        let totalTax = 0;
        ...
        const installmentTaxes = installments.map((inst) => {
            ...
            if (investmentType === 'EQUITY') {
                if (holdingDays >= 365) {
                    ...
                    isLongTerm = true;
                    ltcg += capitalGain;
                    const remainingExemption = Math.max(0, exemptionLimit - ltcgExemptionUsed);
