 ...
 * }
 */
router.post('/compare', auth_middleware_1.optionalAuth, async (req, res) => {
    try {
        let { fundIds, topNHoldings, correlationPeriod, includeCorrelation } = req.body;
        // Resolve search terms to fundIds if needed
        if (typeof fundIds === 'string' ||
            (Array.isArray(fundIds) &&
                fundIds.some((id) => typeof id === 'string' && id.includes(' ')))) {
        ...
        const comparisonService = new comparison_service_1.ComparisonService(db);
        // Perform comparison
        ...
        // Cache for 1 hour
        await redis_1.redis.set(cacheKey, comparison, 60 * 60);
        res.json({
            success: true,
            data: comparison,
            cached: false,
 ...
 * }
 */
router.post('/overlap', auth_middleware_1.optionalAuth, async (req, res) => {
    try {
 ...
 */
router.get('/history', auth_middleware_1.optionalAuth, async (req, res) => {
    try {
        if (!req.user) {
 ...
 */
function calculateDetailedHoldingsOverlap(funds) {
 ...
 */
function calculateDetailedSectorOverlap(funds) {
 ...
 */
function findDuplicateExposure(funds) {
    ...
    for (const fund of funds) {
        if (fund.holdings) {
            for (const holding of fund.holdings) {
                const name = holding.name?.toLowerCase();
                if (name) {
                    exposureMap.set(name, (exposureMap.get(name) || 0) + (holding.percentage || 0));
                }
            }
        }
    }
 ...
 */
function generateOverlapWarnings(holdingsOverlap, sectorOverlap) {
    const warnings = [];
    if (holdingsOverlap.common > 10) {
        warnings.push(`High holdings overlap detected: ${holdingsOverlap.common} common holdings`);
    }
    const highSectorOverlap = sectorOverlap.find((s) => s.avgWeight > 30);
    if (highSectorOverlap) {
        warnings.push(`Heavy concentration in ${highSectorOverlap.sector} sector (${highSectorOverlap.avgWeight.toFixed(1)}%)`);
    }
    return warnings;
}
