...
exports.enrichFundData = enrichFundData;
/**
 ...
 */
function calculateAnnualizedReturn(startNav, endNav, years) {
    if (!startNav || startNav === 0 || years === 0)
        return 0;
 ...
 */
function getNavAtDate(performances, targetDate) {
    if (!performances || performances.length === 0)
        return null;
    const sorted = [...performances].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    let closest = sorted[0];
    let minDiff = Math.abs(new Date(sorted[0].date).getTime() - targetDate.getTime());
 ...
 */
function calculateReturns(performances, currentNav) {
    if (!performances || performances.length === 0) {
        return {
            oneYear: 0,
            threeYear: 0,
 ...
 */
function calculateDailyReturns(performances) {
 ...
 */
function calculateSharpeRatio(portfolioReturn, standardDeviation, riskFreeRate = 6.5) {
 ...
 */
function calculateBeta(fundVolatility, benchmarkVolatility = 15) {
 ...
 */
function calculateAlpha(portfolioReturn, beta, riskFreeRate = 6.5, marketReturn = 12) {
 ...
 */
function calculateRiskMetrics(performances, returns) {
 ...
 */
function determineRiskLevel(volatility, category) {
 ...
 */
function calculateRating(returns, riskMetrics) {
    let score = 0;
    // Return contribution (max 1.6 points)
    if (returns.oneYear > 20)
        score += 1.6;
    else if (returns.oneYear > 15)
        score += 1.2;
    else if (returns.oneYear > 10)
        score += 0.8;
    else if (returns.oneYear > 5)
        score += 0.4;
    // Sharpe ratio contribution (max 1.5 points)
    if (riskMetrics.sharpeRatio > 1.5)
        score += 1.5;
    else if (riskMetrics.sharpeRatio > 1)
        score += 1.2;
    ...
    if (riskMetrics.alpha > 5)
        score += 1.0;
    else if (riskMetrics.alpha > 2)
        score += 0.8;
    else if (riskMetrics.alpha > 0)
        score += 0.4;
    // Consistency bonus (max 0.5 points)
    if (returns.threeYear > 0 &&
        returns.oneYear > 0 &&
