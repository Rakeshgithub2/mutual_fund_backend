...
exports.amfiImporter = exports.AMFIImporter = void 0;
...
class AMFIImporter {
     ...
     */
    async importMutualFunds(options = {}) {
        ...
        try {
            ...
            console.log(`📝 Found ${uniqueFundHouses.length} unique fund houses in data`);
            console.log(`📝 Sample fund house names:`);
            allFunds.slice(0, 5).forEach((f) => {
                console.log(`   "${f.fundHouse}" - ${f.schemeName.substring(0, 40)}...`);
            });
            ...
            const priorityFunds = allFunds.filter((fund) => this.topFundHouses.some((house) => {
                const houseName = house.replace(' Mutual Fund', '');
                return fund.fundHouse.includes(houseName) || fund.fundHouse === house;
            }));
            console.log(`📊 Found ${priorityFunds.length} funds from top fund houses`);
            ...
            for (const amfiFund of selectedFunds) {
                try {
                    ...
                    result.failed++;
                    result.errors.push(`Error importing ${amfiFund.schemeName}: ${error.message}`);
                }
            }
     ...
     */
    selectDiverseFunds(funds, limit) {
        const selected = [];
        const categoryCounts = new Map();
        // Group funds by scheme type
        const fundsByType = new Map();
...
exports.AMFIImporter = AMFIImporter;
// Export singleton instance
exports.amfiImporter = new AMFIImporter();
