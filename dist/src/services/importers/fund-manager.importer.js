...
exports.fundManagerImporter = exports.FundManagerImporter = void 0;
...
class FundManagerImporter {
     ...
     */
    generateManagerFromFund(fundData) {
    ...
    async extractManagers(funds) {
        ...
        for (const fund of funds) {
            try {
                ...
                result.errors.push(`Error extracting manager from ${fund.name}: ${error.message}`);
            }
        }
     ...
     */
    async enrichManagersWithFunds(managers, funds) {
        console.log(`\n🔗 Enriching managers with fund assignments...`);
        for (const manager of managers) {
            const managedFunds = funds.filter((fund) => fund.fundManager === manager.name &&
                fund.fundHouse.includes(manager.fundHouse.replace(' Mutual Fund', '')));
            manager.fundsManaged = managedFunds.map((fund) => ({
                fundName: fund.name,
                fundId: fund.symbol,
            }));
            console.log(`  ✓ ${manager.name}: ${manager.fundsManaged?.length || 0} funds`);
        }
        return managers;
    }
    /**
     ...
     */
    assignManagersToFunds(funds, managers) {
        console.log(`\n🎯 Assigning managers to funds...`);
        for (const fund of funds) {
            if (!fund.fundManager || fund.fundManager === 'N/A') {
                // Find manager from same fund house
                const sameHouseManagers = managers.filter((m) => fund.fundHouse.includes(m.fundHouse.replace(' Mutual Fund', '')));
                if (sameHouseManagers.length > 0) {
                    // Assign random manager from same house
                    const manager = sameHouseManagers[Math.floor(Math.random() * sameHouseManagers.length)];
...
exports.FundManagerImporter = FundManagerImporter;
// Export singleton instance
exports.fundManagerImporter = new FundManagerImporter();
