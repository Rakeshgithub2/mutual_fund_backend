...
exports.enhancedYahooImporter = exports.EnhancedYahooImporter = void 0;
...
class EnhancedYahooImporter {
     ...
     */
    getPriceByDaysAgo(data, daysAgo) {
     ...
     */
    async convertETFToRawFundData(etf) {
        try {
            ...
            if (etf.name.includes('SPDR'))
                fundHouse = 'State Street';
            else if (etf.name.includes('iShares'))
                fundHouse = 'BlackRock';
            else if (etf.name.includes('Vanguard'))
                fundHouse = 'Vanguard';
            else if (etf.name.includes('Invesco'))
                fundHouse = 'Invesco';
            else if (etf.symbol.includes('.NS'))
     ...
     */
    estimateETFExpenseRatio(symbol) {
        if (symbol.includes('Vanguard') || symbol.startsWith('V'))
            return 0.1 + Math.random() * 0.2;
        if (symbol.includes('iShares'))
            return 0.2 + Math.random() * 0.3;
        if (symbol.includes('SPDR'))
     ...
     */
    async importCommodityETFs(options = {}) {
        ...
        try {
            console.log(`📥 Importing ${Math.min(limit, this.commodityETFs.length)} commodity ETFs...`);
     ...
     * Import equity ETFs
     */
    async importEquityETFs(options = {}) {
        const { limit = 30, categories = ['Large Cap', 'Mid Cap', 'Small Cap'] } = options;
        const result = {
            success: true,
            imported: 0,
            failed: 0,
        ...
        try {
            console.log(`📥 Importing equity ETFs for categories: ${categories.join(', ')}...`);
            // Filter ETFs by requested categories
            const filteredETFs = this.equityETFs.filter((etf) => categories.includes(etf.subCategory));
...
exports.EnhancedYahooImporter = EnhancedYahooImporter;
// Export singleton instance
exports.enhancedYahooImporter = new EnhancedYahooImporter();
