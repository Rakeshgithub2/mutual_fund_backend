...
exports.fundIngestionOrchestrator = exports.FundIngestionOrchestrator = void 0;
const yahoo_finance_importer_1 = require("./yahoo-finance.importer");
const amfi_importer_1 = require("./amfi.importer");
const mfapi_importer_1 = require("./mfapi.importer");
const enhanced_yahoo_importer_1 = require("./enhanced-yahoo.importer");
const fund_manager_importer_1 = require("./fund-manager.importer");
const models_1 = require("../../models");
// Lazy-initialized model instances
let fundModel = null;
let fundPriceModel = null;
let fundManagerModel = null;
const getFundModel = () => {
    if (!fundModel)
...
const getFundManagerModel = () => {
    if (!fundManagerModel)
        fundManagerModel = models_1.FundManagerModel.getInstance();
 ...
 */
class FundIngestionOrchestrator {
    /**
     * Enhanced import for real-world fund data
     * Imports equity funds (100+) and commodity funds (50+) from real APIs
     */
    async importRealWorldFunds(options = {}) {
        const { equityCount = 100, commodityCount = 50, skipExisting = true, } = options;
        console.log('\n');
        console.log('════════════════════════════════════════════════════');
        ...
        try {
            ...
            console.log('─────────────────────────────────────────────────\n');
            const equityResult = await mfapi_importer_1.mfApiImporter.importEquityFunds({
                limit: equityCount,
                targetCounts: {
            ...
            console.log('─────────────────────────────────────────────────\n');
            const commodityMFResult = await mfapi_importer_1.mfApiImporter.importCommodityFunds({
            ...
            console.log('─────────────────────────────────────────────────\n');
            const commodityETFResult = await enhanced_yahoo_importer_1.enhancedYahooImporter.importCommodityETFs({
            ...
            console.log('─────────────────────────────────────────────────\n');
            const managerResult = await fund_manager_importer_1.fundManagerImporter.extractManagers(allFunds);
            const knownManagers = fund_manager_importer_1.fundManagerImporter.getAllKnownManagers();
            ...
            console.log('─────────────────────────────────────────────────\n');
            const fundsWithManagers = fund_manager_importer_1.fundManagerImporter.assignManagersToFunds(allFunds, managerResult.data);
     ...
     */
    async importAllFunds(options = {}) {
        const { etfLimit = 50, mutualFundLimit = 100, skipExisting = true, } = options;
        console.log('\n');
        console.log('════════════════════════════════════════════════════');
        ...
        try {
            ...
            if (etfLimit > 0) {
                ...
                console.log('─────────────────────────────────────────────────\n');
                etfResult = await yahoo_finance_importer_1.yahooFinanceImporter.importETFs({
                    limit: etfLimit,
                    skipExisting,
                });
                console.log(`\n✅ ETF Import Results:`);
            ...
            console.log('─────────────────────────────────────────────────\n');
            const mfResult = await amfi_importer_1.amfiImporter.importMutualFunds({
     ...
     */
    async saveManagers(rawManagers) {
        let saved = 0;
        for (const rawManager of rawManagers) {
            try {
                ...
                console.error(`Error saving manager ${rawManager.name}:`, error.message);
            }
        }
        return saved;
    }
     ...
     */
    async importHistoricalPrices(fundIds, period = '1y') {
        ...
        for (const fundId of fundIds) {
            try {
                ...
                const historicalData = await yahoo_finance_importer_1.yahooFinanceImporter.importHistoricalPrices(fundId, period);
                // Save to database
                for (const priceData of historicalData) {
                    if (!priceData.close)
                        continue;
                    const previousClose = priceData.open || priceData.close;
                    const changePercent = ((priceData.close - previousClose) / previousClose) * 100;
                ...
                console.error(`  ✗ Error importing prices for ${fundId}:`, error.message);
            }
        }
...
exports.FundIngestionOrchestrator = FundIngestionOrchestrator;
// Export singleton instance
exports.fundIngestionOrchestrator = new FundIngestionOrchestrator();
