...
exports.ComprehensiveAMFIImporter = void 0;
 ...
 */
class ComprehensiveAMFIImporter {
    constructor() {
        this.navUrl = 'https://www.amfiindia.com/spages/NAVAll.txt';
     ...
     */
    async importCompleteFunds(options = {}) {
        const { onlyFromAMCs = [], skipValidation = false, enforceSEBICategories = true, } = options;
        ...
        try {
            ...
            if (onlyFromAMCs.length > 0) {
                filteredFunds = rawFunds.filter((f) => onlyFromAMCs.includes(f.fundHouse));
                console.log(`🔍 Filtered to ${filteredFunds.length} funds from specified AMCs`);
            }
            ...
            for (const rawFund of filteredFunds) {
                try {
                    ...
                    const { schemeType, planType } = this.extractSchemeAndPlan(rawFund.schemeName);
                    // Calculate data completeness
                    const completeness = this.calculateDataCompleteness(rawFund);
                    // Build fund object
                    const fund = {
                        fundId: rawFund.isinGrowth ||
                            rawFund.isinDiv ||
                        ...
                        visibilityReason: completeness.completenessScore < 60
                    ...
                    if (existingFund) {
                        ...
                        await fundsCollection.updateOne({ _id: existingFund._id }, { $set: { ...fund, updatedAt: new Date() } });
                        result.fundsUpdated++;
                    }
                    else {
                        // Insert new fund
                    ...
                    processedFunds.push(fund);
                }
                catch (error) {
                    result.errors.push(`Error processing ${rawFund.schemeName}: ${error.message}`);
                    result.fundsSkipped++;
                }
            }
            result.success = true;
     ...
     */
    async fetchNAVData() {
        const response = await axios_1.default.get(this.navUrl, {
     ...
     */
    extractSchemeAndPlan(fundName) {
        const lowerName = fundName.toLowerCase();
        const schemeType = lowerName.includes('direct')
            ? 'direct'
            : 'regular';
        let planType = 'growth';
     ...
     */
    calculateDataCompleteness(rawFund) {
        let score = 0;
        const checks = {
            hasNAV: false,
            hasDate: false,
...
exports.ComprehensiveAMFIImporter = ComprehensiveAMFIImporter;
