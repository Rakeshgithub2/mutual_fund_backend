...
exports.sendKYCRejection = exports.sendKYCApproval = exports.sendKYCSubmission = exports.kycEmailService = exports.KYCEmailService = void 0;
...
class KYCEmailService {
    ...
    async sendKYCSubmission(data) {
        if (!this.resend) {
            console.warn('Email service not configured - skipping KYC submission email');
        ...
        try {
            ...
            console.log(`KYC submission email sent to ${data.userEmail}:`, result.data?.id);
            return { success: true };
        }
        catch (error) {
            console.error(`Failed to send KYC submission email to ${data.userEmail}:`, error);
            return {
                success: false,
    ...
    async sendKYCApproval(data) {
        if (!this.resend) {
            console.warn('Email service not configured - skipping KYC approval email');
            return { success: false, error: 'Email service not configured' };
        }
        try {
            ...
            console.log(`KYC approval email sent to ${data.userEmail}:`, result.data?.id);
            return { success: true };
        }
        catch (error) {
            console.error(`Failed to send KYC approval email to ${data.userEmail}:`, error);
            return {
                success: false,
                error: error instanceof Error ? error.message : 'Unknown error',
            };
    ...
    async sendKYCRejection(data) {
        if (!this.resend) {
            console.warn('Email service not configured - skipping KYC rejection email');
            return { success: false, error: 'Email service not configured' };
        }
        try {
            ...
            console.log(`KYC rejection email sent to ${data.userEmail}:`, result.data?.id);
            return { success: true };
        }
        catch (error) {
            console.error(`Failed to send KYC rejection email to ${data.userEmail}:`, error);
            return {
                success: false,
                error: error instanceof Error ? error.message : 'Unknown error',
            };
    ...
    async sendKYCReminder(userEmail, userName) {
        if (!this.resend) {
            console.warn('Email service not configured - skipping KYC reminder email');
            return { success: false, error: 'Email service not configured' };
        }
        try {
            const html = `
      ...
      `;
            ...
            console.error(`Failed to send KYC reminder email to ${userEmail}:`, error);
            return {
                success: false,
                error: error instanceof Error ? error.message : 'Unknown error',
            };
        ...
        <div class="container">
          ...
          <div class="content">
            ...
            <div class="info-box">
              ...
              <p><strong>Submitted On:</strong> ${data.submissionDate.toLocaleDateString('en-IN', {
            year: 'numeric',
            month: 'long',
            day: 'numeric',
            hour: '2-digit',
        ...
        <div class="container">
          ...
          <div class="content">
            ...
            <div class="info-box">
              ...
              <p><strong>Approved On:</strong> ${data.approvalDate.toLocaleDateString('en-IN', {
            year: 'numeric',
            month: 'long',
            day: 'numeric',
        })}</p>
...
exports.KYCEmailService = KYCEmailService;
exports.kycEmailService = new KYCEmailService();
// Backwards-compatible wrappers used by controllers
const sendKYCSubmission = async (userEmail, userName) => {
    return exports.kycEmailService.sendKYCSubmission({
        userEmail,
        userName,
...
exports.sendKYCSubmission = sendKYCSubmission;
const sendKYCApproval = async (userEmail, userName) => {
    return exports.kycEmailService.sendKYCApproval({
        userEmail,
        userName,
        approvalDate: new Date(),
    });
};
exports.sendKYCApproval = sendKYCApproval;
const sendKYCRejection = async (userEmail, userName, reason) => {
    return exports.kycEmailService.sendKYCRejection({
        userEmail,
        userName,
        reason,
...
exports.sendKYCRejection = sendKYCRejection;
