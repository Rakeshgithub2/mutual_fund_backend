...
exports.fundDataService = exports.ComprehensiveFundDataService = void 0;
...
const CONFIG = {
    // India - AMFI
    AMFI_NAV_URL: process.env.AMFI_NAV_URL || 'https://www.amfiindia.com/spages/NAVAll.txt',
    // India - NSE
    NSE_API_BASE: 'https://www.nseindia.com/api',
    NSE_ETF_LIST: 'https://www.nseindia.com/api/equity-stockIndices?index=NIFTY%20ETF',
    // Yahoo Finance via RapidAPI
    RAPIDAPI_KEY: process.env.RAPIDAPI_KEY,
    RAPIDAPI_HOST: process.env.RAPIDAPI_HOST || 'apidojo-yahoo-finance-v1.p.rapidapi.com',
    // Alpha Vantage
...
async function rateLimitedCall(source) {
    const now = Date.now();
...
class ComprehensiveFundDataService {
    constructor(db) {
        this.db = db;
    }
    // ==================== AMFI DATA FETCHING ====================
    ...
    async fetchAMFIData() {
        ...
        try {
            const response = await axios_1.default.get(CONFIG.AMFI_NAV_URL, {
                timeout: 60000,
                headers: {
            ...
            for (const line of lines) {
                ...
                if (trimmed.includes(';')) {
                    ...
                    if (parts.length >= 6) {
                        const [schemeCode, isinDiv, isinGrowth, schemeName, navStr, dateStr,] = parts;
        ...
        try {
            ...
            for (const etf of etfs) {
                ...
                try {
                    ...
                    fundsToUpsert.push(fund);
                    // Save price
                    await this.savePriceData(fund.fundId, fund.currentNav, new Date(), fund.returns.day);
                }
                catch (error) {
        ...
        for (const symbol of symbols) {
            try {
                ...
                const fund = {
                    ...
                    navDate: new Date((quote.regularMarketTime || Date.now() / 1000) * 1000),
                    ...
                    returns: {
                        ...
                        week: this.calculateReturn(quote.regularMarketPrice, stats.fiftyTwoWeekLow, 7),
                        month: this.calculateReturn(quote.regularMarketPrice, stats.fiftyTwoWeekLow, 30),
                        threeMonth: this.calculateReturn(quote.regularMarketPrice, stats.fiftyTwoWeekLow, 90),
                        sixMonth: this.calculateReturn(quote.regularMarketPrice, stats.fiftyTwoWeekLow, 180),
                ...
                fundsToUpsert.push(fund);
                await this.savePriceData(symbol, fund.currentNav, fund.navDate, fund.returns.day);
            }
            catch (error) {
    ...
    async fetchHistoricalData(symbol, outputSize = 'compact') {
        ...
        try {
            ...
            for (const [dateStr, data] of Object.entries(timeSeries)) {
                const close = parseFloat(data['5. adjusted close']);
                const prevClose = parseFloat(data['4. close']);
            ...
            for (const price of prices) {
                await this.savePriceData(symbol, price.nav, price.date, price.changePercent);
            }
            console.log(`✅ [Alpha Vantage] Saved ${prices.length} historical prices for ${symbol}`);
        }
        catch (error) {
     ...
     */
    async enrichFundManagerData(fundId) {
        console.log(`📊 Enriching manager data for ${fundId}`);
        const fund = await this.db.collection('funds').findOne({ fundId });
        if (!fund)
            return;
        ...
        if (managerInfo) {
            const manager = {
                managerId: (0, crypto_1.randomUUID)(),
            ...
            await this.db.collection('funds').updateOne({ fundId }, {
                $set: {
     ...
     */
    async scrapeFundManager(fundHouse, fundName) {
    ...
    async fetchYahooProfile(symbol) {
        try {
            const options = {
                method: 'GET',
    ...
    async fetchYahooHoldings(symbol) {
        try {
            const options = {
                method: 'GET',
    ...
    async savePriceData(fundId, nav, date, changePercent = 0) {
        const pricesCollection = this.db.collection('fundPrices');
        const dateOnly = new Date(date.setHours(0, 0, 0, 0));
        await pricesCollection.updateOne({ fundId, date: dateOnly }, {
            $set: {
                fundId,
                date: dateOnly,
                nav,
    ...
    categorizeFromYahoo(quote, profile) {
        if (quote.quoteType === 'COMMODITY')
            return 'commodity';
    ...
    subCategorizeFromYahoo(quote, profile) {
        const name = (quote.longName || quote.shortName || '').toLowerCase();
        if (name.includes('gold'))
            return 'Gold';
        if (name.includes('silver'))
            return 'Silver';
        if (name.includes('oil') || name.includes('energy'))
            return 'Oil & Gas';
        if (name.includes('agriculture'))
            return 'Agriculture';
        return 'Other';
    }
    estimateExpenseRatio(name) {
        if (name.toLowerCase().includes('index'))
            return 0.5;
        if (name.toLowerCase().includes('etf'))
            return 0.5;
        if (name.toLowerCase().includes('equity'))
            return 2.0;
        if (name.toLowerCase().includes('debt'))
            return 1.0;
        return 1.5;
    }
    estimateExitLoad(name) {
        if (name.toLowerCase().includes('liquid'))
            return 0;
        if (name.toLowerCase().includes('etf'))
            return 0;
        return 1.0;
    ...
    calculateReturn(current, past, days) {
        if (!past || past === 0)
            return 0;
        return ((current - past) / past) * 100;
    }
    calculateDayReturn(priceInfo) {
        if (!priceInfo)
            return 0;
...
exports.ComprehensiveFundDataService = ComprehensiveFundDataService;
// Export default instance
const fundDataService = (db) => new ComprehensiveFundDataService(db);
