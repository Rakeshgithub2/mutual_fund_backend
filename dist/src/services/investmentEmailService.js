...
exports.sendInvestmentFailure = exports.sendSIPConfirmation = exports.sendInvestmentConfirmation = exports.investmentEmailService = exports.InvestmentEmailService = void 0;
...
class InvestmentEmailService {
    ...
    async sendInvestmentConfirmation(data) {
        if (!this.resend) {
            console.warn('Email service not configured - skipping investment confirmation email');
        ...
        try {
            ...
            console.log(`Investment confirmation email sent to ${data.userEmail}:`, result.data?.id);
            return { success: true };
        }
        catch (error) {
            console.error(`Failed to send investment confirmation to ${data.userEmail}:`, error);
            return {
                success: false,
    ...
    async sendSIPConfirmation(data) {
        if (!this.resend) {
            console.warn('Email service not configured - skipping SIP confirmation email');
            return { success: false, error: 'Email service not configured' };
        }
        try {
            ...
            console.log(`SIP confirmation email sent to ${data.userEmail}:`, result.data?.id);
            return { success: true };
        }
        catch (error) {
            console.error(`Failed to send SIP confirmation to ${data.userEmail}:`, error);
            return {
                success: false,
                error: error instanceof Error ? error.message : 'Unknown error',
            };
    ...
    async sendInvestmentFailure(userEmail, userName, fundName, reason) {
        if (!this.resend) {
            console.warn('Email service not configured - skipping investment failure email');
            return { success: false, error: 'Email service not configured' };
        }
        try {
            const html = `
      ...
      `;
            ...
            console.log(`Investment failure email sent to ${userEmail}:`, result.data?.id);
            return { success: true };
        }
        catch (error) {
            console.error(`Failed to send investment failure email to ${userEmail}:`, error);
            return {
                success: false,
                error: error instanceof Error ? error.message : 'Unknown error',
            };
    ...
    getInvestmentConfirmationTemplate(data) {
        return `
      <!DOCTYPE html>
      <html>
      <head>
    ...
    getSIPConfirmationTemplate(data) {
        return `
      <!DOCTYPE html>
      <html>
      <head>
...
exports.InvestmentEmailService = InvestmentEmailService;
exports.investmentEmailService = new InvestmentEmailService();
...
const sendInvestmentConfirmation = async (userEmail, userName, payload) => {
    return exports.investmentEmailService.sendInvestmentConfirmation({
        userEmail,
        userName,
...
exports.sendInvestmentConfirmation = sendInvestmentConfirmation;
const sendSIPConfirmation = async (userEmail, userName, payload) => {
    return exports.investmentEmailService.sendSIPConfirmation({
        userEmail,
        userName,
        fundName: payload.fundName,
...
exports.sendSIPConfirmation = sendSIPConfirmation;
const sendInvestmentFailure = async (userEmail, userName, fundNameOrReason, reason) => {
    ...
    return exports.investmentEmailService.sendInvestmentFailure(userEmail, userName, fundName, finalReason);
};
exports.sendInvestmentFailure = sendInvestmentFailure;
