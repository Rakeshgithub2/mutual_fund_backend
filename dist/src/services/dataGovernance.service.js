...
class DataGovernanceService {
     ...
     */
    async initialize(db) {
        this.fundsCollection = db.collection('funds');
        this.pricesCollection = db.collection('fund_prices');
        console.log('✅ Data Governance Service initialized');
    }
    /**
     * Comprehensive validation for a single fund
     */
    async validateFund(fundId) {
    ...
    async validateAllFunds() {
        ...
        console.log(`✅ Validated ${funds.length} funds, found ${results.length} with issues`);
        return results;
    }
    /**
     * NAV Validation
    ...
    validateNAV(fund) {
        ...
        const navChange = Math.abs((fund.currentNav - fund.previousNav) / fund.previousNav) * 100;
        if (navChange > 10) {
            issues.push({
                severity: 'warning',
                category: 'nav',
     ...
     */
    validateReturnsConsistency(fund) {
        const issues = [];
        ...
        // Check 1-year return
        if (fund.returns.oneYear < expectedRange.min ||
        ...
        if (fund.returns.oneYear && fund.returns.threeYear) {
            const returnDiff = Math.abs(fund.returns.oneYear - fund.returns.threeYear);
            if (returnDiff > 30) {
                issues.push({
                    severity: 'info',
                    category: 'returns',
    ...
    validateHoldings(fund) {
        ...
        const totalPercentage = fund.holdings.reduce((sum, h) => sum + h.percentage, 0);
     ...
     */
    checkMetricOutlier(fund, peers, metric) {
        const values = peers
     ...
     */
    async generateFreshnessReport() {
        if (!this.fundsCollection) {
            throw new Error('Data governance service not initialized');
        }
        const funds = await this.fundsCollection.find({ isActive: true }).toArray();
        const reports = funds.map((fund) => {
            const navAge = this.calculateAgeInDays(fund.navDate);
            const aumAge = this.calculateAgeInDays(fund.aumDate);
            const returnsAge = this.calculateAgeInDays(fund.lastUpdated);
            let overallFreshness = 'fresh';
    ...
    async autoHideIncompleteFunds() {
        ...
        for (const fund of incompleteFunds) {
            ...
            await this.fundsCollection.updateOne({ fundId: fund.fundId }, {
                $set: {
        ...
        console.log(`✅ Auto-hidden ${hiddenCount} funds with insufficient data quality`);
        return hiddenCount;
    }
    // ==================== UTILITY METHODS ====================
    /**
