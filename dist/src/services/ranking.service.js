...
class RankingService {
     ...
     */
    async getTopFunds(limit = 20, criteria = {}) {
     ...
     */
    async getCategoryLeaders(category, limit = 10) {
        const cacheKey = `category_${category}_${limit}`;
        const cached = this.getCachedRanking(cacheKey);
        if (cached)
            return cached;
        const criteria = { category, limit };
        const funds = await this.getEligibleFunds(criteria);
        const rankedFunds = await this.calculateRankings(funds, criteria);
     ...
     */
    async getRiskAdjustedRankings(limit = 50, criteria = {}) {
     ...
     */
    async getRollingReturnRankings(period, limit = 100, criteria = {}) {
        ...
        rankedFunds.sort((a, b) => {
            const aReturn = period === '2y'
                ? a.returns.twoYear
                : period === '3y'
                    ? a.returns.threeYear
                    : a.returns.fiveYear;
            const bReturn = period === '2y'
                ? b.returns.twoYear
                : period === '3y'
                    ? b.returns.threeYear
                    : b.returns.fiveYear;
     ...
     * @param limit Number of funds
     */
    async getSubCategoryLeaders(category, subCategory, limit = 10) {
        const cacheKey = `subcategory_${category}_${subCategory}_${limit}`;
        const cached = this.getCachedRanking(cacheKey);
        if (cached)
            return cached;
        const criteria = { category, subCategory, limit };
        const funds = await this.getEligibleFunds(criteria);
        const rankedFunds = await this.calculateRankings(funds, criteria);
        rankedFunds.sort((a, b) => b.overallScore - a.overallScore);
        rankedFunds.forEach((fund, index) => {
     ...
     */
    async getEligibleFunds(criteria) {
     ...
     */
    async calculateRankings(funds, criteria) {
        const rankedFunds = funds.map((fund) => {
     ...
     */
    calculatePerformanceScore(fund, criteria) {
        let score = 0;
        let weights = 0;
     ...
     */
    normalizeReturn(returnValue, category) {
        ...
        // Score calculation
        if (returnValue >= range.excellent)
            return 100;
        if (returnValue <= range.min)
            return 0;
        // Linear interpolation
        const normalized = ((returnValue - range.min) / (range.excellent - range.min)) * 100;
     ...
     */
    calculateRiskAdjustedScore(fund) {
        let score = 0;
     ...
     */
    calculateConsistencyScore(fund) {
        const returns = [
