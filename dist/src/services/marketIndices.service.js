...
exports.marketIndicesService = exports.MarketIndicesService = void 0;
...
class MarketIndicesService {
    constructor() {
        ...
        this.NSE_API = 'https://www.nseindia.com/api/allIndices';
        this.BSE_API = 'https://api.bseindia.com/BseIndiaAPI/api/DefaultData/w';
        // Fallback APIs
        this.YAHOO_FINANCE_API = 'https://query1.finance.yahoo.com/v8/finance/chart';
        // Staleness threshold (minutes)
        this.STALENESS_THRESHOLD = 15;
     ...
     */
    async getAllIndices() {
        const indicesCollection = mongodb_1.mongodb.getCollection('marketIndices');
        ...
        const freshIndices = cachedIndices.filter((idx) => {
            const ageMinutes = (now.getTime() - idx.lastUpdated.getTime()) / 1000 / 60;
            return ageMinutes < 5;
        });
     ...
     */
    async fetchAndUpdateAllIndices() {
        const indianIndices = [
            'NIFTY_50',
            'NIFTY_NEXT_50',
            'NIFTY_MIDCAP_100',
     ...
     */
    async fetchAndUpdateIndex(indexId) {
        try {
            ...
            if (!sanityPassed) {
                console.warn(`Sanity check failed for ${indexId}: ${indexData.changePercent}% change`);
            }
            // Calculate staleness
            indexData.staleness = this.calculateStaleness(indexData.lastUpdated);
            // Save to database
            const indicesCollection = mongodb_1.mongodb.getCollection('marketIndices');
            await indicesCollection.updateOne({ indexId: indexData.indexId }, { $set: indexData }, { upsert: true });
            return indexData;
        }
        catch (error) {
     ...
     */
    async fetchFromPrimarySource(indexId) {
        try {
            const response = await axios_1.default.get(this.NSE_API, {
                headers: {
            ...
            if (!targetName)
                return null;
            const indexData = data.data?.find((idx) => idx.index === targetName);
            if (!indexData)
                return null;
            ...
            const perChange = parseFloat(indexData.perChange) ||
     ...
     */
    async fetchFromYahooFinance(indexId) {
        try {
            const symbolMap = {
                // Indian Indices
            ...
            const chartPreviousClose = parseFloat(meta.chartPreviousClose) || regularMarketPrice;
            const change = regularMarketPrice - chartPreviousClose;
            const changePercent = chartPreviousClose > 0 ? (change / chartPreviousClose) * 100 : 0;
            return {
                indexId,
                name: indexInfo.name,
    ...
    performSanityCheck(indexData) {
        ...
        const isValid = indexData.changePercent >= range.min &&
            indexData.changePercent <= range.max;
        if (!isValid) {
            console.log(`Sanity check failed for ${indexData.indexId}: ${indexData.changePercent.toFixed(2)}% outside range ${range.min}% to ${range.max}%`);
        }
        return isValid;
    }
    /**
     ...
     */
    calculateStaleness(lastUpdated) {
        const now = new Date();
...
exports.marketIndicesService = MarketIndicesService.getInstance();
