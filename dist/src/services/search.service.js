...
class SearchService {
    ...
    async search(query, options = {}) {
        const { limit = 10, category, minAum, enableFuzzy = true, boostPopular = true, } = options;
        ...
        const exactMatches = await this.exactMatch(normalizedQuery, category, minAum);
        results.push(...exactMatches.map((r) => ({
            ...r,
            score: r.score * 1.5,
            matchType: 'exact',
        })));
        ...
        if (results.length < limit) {
            const prefixMatches = await this.prefixMatch(normalizedQuery, category, minAum);
            results.push(...prefixMatches.map((r) => ({
                ...r,
                score: r.score * 1.2,
                matchType: 'prefix',
            })));
        ...
        if (enableFuzzy && results.length < limit && normalizedQuery.length >= 3) {
            const fuzzyMatches = await this.fuzzyMatch(normalizedQuery, category, minAum);
            results.push(...fuzzyMatches.map((r) => ({ ...r, matchType: 'fuzzy' })));
        }
        // Layer 4: Intent-based tag search
        if (results.length < limit) {
            const tagMatches = await this.intentBasedSearch(normalizedQuery, category, minAum);
            results.push(...tagMatches.map((r) => ({
                ...r,
                score: r.score * 0.8,
                matchType: 'tag',
            })));
        ...
        await redis_1.redis.set(cacheKey, finalResults, 30 * 60);
        return finalResults;
    }
    /**
    ...
    async suggest(prefix, limit = 10) {
        ...
        if (wordCount === 1) {
            ...
            if (environment_1.config.features.enableAtlasSearch) {
        ...
        suggestions = suggestions.map((s) => this.enrichAutocompleteResult(s, tokens, normalizedPrefix));
        // Sort by confidence
        suggestions.sort((a, b) => (b.confidence || 0) - (a.confidence || 0));
        // Cache for 1 hour
        await redis_1.redis.set(cacheKey, suggestions, 60 * 60);
        return suggestions.slice(0, limit);
    }
    // ==================== LAYER 1: EXACT MATCH ====================
    async exactMatch(query, category, minAum) {
        const filter = {
            isActive: true,
            $or: [
    ...
    // ==================== LAYER 2: PREFIX MATCH ====================
    async prefixMatch(query, category, minAum) {
        const filter = {
            isActive: true,
            $or: [
                { name: { $regex: `^${this.escapeRegex(query)}`, $options: 'i' } },
    ...
    // ==================== LAYER 3: FUZZY MATCH ====================
    async fuzzyMatch(query, category, minAum) {
        if (environment_1.config.features.enableAtlasSearch) {
        ...
        const fuzzyResults = results
            .map((result) => {
            const nameDistance = this.levenshteinDistance(query, result.name.toLowerCase());
            const fundHouseDistance = this.levenshteinDistance(query, result.fundHouse.toLowerCase());
            const minDistance = Math.min(nameDistance, fundHouseDistance);
            // Accept if within 2 edits
            const fuzzyScore = minDistance <= 2 ? (1 - minDistance / query.length) * 100 : 0;
            return {
                ...result,
                score: fuzzyScore,
            };
    ...
    async intentBasedSearch(query, category, minAum) {
    ...
    async atlasAutocomplete(prefix, limit) {
        const results = await this.db
            .collection('funds')
            .aggregate([
            {
    ...
    async atlasFuzzySearch(query, category, minAum) {
        const matchStage = { isActive: true };
        if (category)
            matchStage.category = category;
        if (minAum)
            matchStage.aum = { $gte: minAum };
        const results = await this.db
            .collection('funds')
            .aggregate([
            {
    ...
    // ==================== N-GRAM SUGGESTIONS ====================
    async ngramSuggest(prefix, limit) {
     ...
     */
    mapToSearchResults(results, baseScore = 50) {
        return results.map((result) => ({
     ...
     */
    async suggestSingleWord(word, limit) {
        const results = [];
        ...
        if (tags.length > 0 && results.length < limit) {
            const tagResults = await this.db
                ...
                .project(this.getProjection())
                .toArray();
            results.push(...this.mapToSearchResults(tagResults, 80).map((r) => ({
                ...r,
     ...
     */
    async suggestTwoWords(word1, word2, limit) {
        const results = [];
        ...
        const categoryMatch = categoryKeywords[word2];
     ...
     */
    enrichAutocompleteResult(result, tokens, originalQuery) {
        ...
        const matchedTokens = tokens.filter((token) => result.name.toLowerCase().includes(token));
        return {
            ...result,
            confidence,
            highlightedName,
     ...
     */
    calculateConfidence(result, tokens, originalQuery) {
        let confidence = 0;
