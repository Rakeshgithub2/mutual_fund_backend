...
exports.SectorAllocationService = void 0;
...
class SectorAllocationService {
     ...
     */
    static generateFromHoldings(holdings) {
        ...
        const allocation = Array.from(sectorMap.entries()).map(([sector, percentage]) => ({
            sector,
     ...
     */
    static inferSector(companyName) {
        ...
        if (lower.includes('energy') ||
            ...
            lower.includes('gas'))
            return 'Energy';
        if (lower.includes('auto') || lower.includes('motor'))
            return 'Automobile';
        if (lower.includes('cement') ||
            lower.includes('steel') ||
            lower.includes('metal'))
            return 'Materials';
        if (lower.includes('fmcg') ||
            lower.includes('consumer goods') ||
            lower.includes('unilever') ||
            lower.includes('britannia'))
            return 'FMCG';
        if (lower.includes('telecom') || lower.includes('airtel'))
            return 'Telecom';
     ...
     */
    static async fetchAndGenerateSectors(fundId) {
        try {
            ...
            console.log(`✅ Generated ${sectorAllocation.length} sectors from ${holdings.length} holdings`);
     ...
     */
    static async fetchFromRapidAPI(fundId) {
        try {
            ...
            const response = await axios_1.default.get(`https://latest-mutual-fund-nav.p.rapidapi.com/fetchPortfolio`, {
                headers: {
            ...
            const holdings = response.data.holdings
                ...
                sector: this.inferSector(h.company_name || h.companyName || 'Unknown'),
     ...
     */
    static async processSingleFund(fundId) {
        try {
            ...
            // Update database
            await fundsCollection.updateOne({ fundId }, {
                $set: {
     ...
     */
    static async processAllFunds(limit = 100) {
        try {
            const db = mongodb_1.mongodb.getDb();
            const fundsCollection = db.collection('funds');
    ...
    static async getStats() {
        ...
        const coveragePercent = totalEquityFunds > 0 ? (fundsWithSectors / totalEquityFunds) * 100 : 0;
        return {
            totalEquityFunds,
            fundsWithSectors,
            fundsWithoutSectors,
...
exports.SectorAllocationService = SectorAllocationService;
