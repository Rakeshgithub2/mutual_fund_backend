...
class Scheduler {
    ...
    setupJobs() {
        ...
        const amfiJob = new cron_1.CronJob('0 3 * * *', // 3:00 AM daily
        async () => {
            console.log('Enqueueing AMFI ingestion job...');
            try {
                await (0, queues_1.enqueueAMFIIngest)({
        ...
        const yahooJob = new cron_1.CronJob('0 1 * * *', // 1:00 AM daily
        async () => {
            ...
            try {
                ...
                for (const symbol of benchmarks) {
                    await (0, queues_1.enqueueYahooIngest)({
                        symbol,
                        from: fromDateStr,
                        to: toDateStr,
                    });
                }
                console.log(`Yahoo Finance ingestion jobs enqueued for ${benchmarks.length} benchmarks`);
            }
            catch (error) {
                console.error('Failed to enqueue Yahoo Finance ingestion jobs:', error);
            }
        }, null, false, 'Asia/Kolkata');
        // NEWS_INGEST - Hourly during market hours (9:15 AM to 3:30 PM IST, Mon-Fri)
        const newsJob = new cron_1.CronJob('0 9-15 * * 1-5', // Every hour from 9 AM to 3 PM, Monday to Friday
        async () => {
            ...
            if (currentHour >= 9 && currentHour <= 15) {
                ...
                try {
                    await (0, queues_1.enqueueNewsIngest)({
                        category: 'business',
                        keywords: [
                            'mutual fund',
                            'investment',
        ...
        const reminderJob = new cron_1.CronJob('*/5 * * * *', // Every 5 minutes
        async () => {
            console.log('Enqueueing reminder check job...');
            try {
                await (0, queues_1.enqueueReminderCheck)({});
        ...
        const digestJob = new cron_1.CronJob('0 8 * * 1-5', // 8:00 AM Monday to Friday
        async () => {
