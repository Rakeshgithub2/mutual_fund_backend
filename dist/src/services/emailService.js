...
class EmailService {
    ...
    async sendFeedbackNotification(data) {
        ...
        if (!emailUser ||
            ...
            console.warn('⚠️  Email credentials not configured - feedback email will not be sent');
            return { success: false, error: 'Email service not configured' };
        }
        try {
            // Create transporter
            ...
            const ratingText = data.rating > 0 ? `${data.rating}/5 ${stars}` : 'No rating';
              ...
              <div class="content">
                <div class="info-box">
                  ...
                  <div class="info-row">
                    ...
                    <span class="info-value">${new Date(data.timestamp).toLocaleString('en-US', {
                    dateStyle: 'full',
                    timeStyle: 'long',
                })}</span>
                  </div>
     ...
     */
    async sendPasswordResetOTP(to, data) {
        if (!this.resend) {
