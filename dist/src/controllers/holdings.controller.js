...
const FundHoldings_model_1 = __importDefault(require("../models/FundHoldings.model"));
const holdingsFallback_service_1 = require("../services/holdingsFallback.service");
...
class HoldingsController {
    ...
    static async getHoldingsBySchemeCode(req, res) {
        try {
            ...
            // Get latest holdings
            const holdings = await FundHoldings_model_1.default.getLatestHoldings(schemeCode);
            ...
            if (!holdings || holdings.length === 0) {
                ...
                const fallbackData = (0, holdingsFallback_service_1.getFallbackHoldings)(schemeCode, fund.schemeName || fund.name, fund.category, fund.subCategory);
                const limitedHoldings = fallbackData.holdings.slice(0, parseInt(limit));
                const totalWeight = limitedHoldings.reduce((sum, h) => sum + (h.weight || 0), 0);
                const response = {
                    schemeCode,
    ...
    static async getTopHoldings(req, res) {
        try {
            ...
            const holdings = await FundHoldings_model_1.default.getTopHoldings(schemeCode, parseInt(limit));
    ...
    static async getSectorAllocation(req, res) {
        try {
            ...
            const sectors = await FundHoldings_model_1.default.getSectorAllocation(schemeCode);
    ...
    static async getHoldingsStats(req, res) {
        try {
            const stats = await FundHoldings_model_1.default.aggregate([
                {
                    $group: {
                        _id: null,
    ...
    static async compareHoldings(req, res) {
        try {
            ...
            const holdingsData = await Promise.all(schemeCodes.map(async (code) => {
                const holdings = await FundHoldings_model_1.default.getLatestHoldings(code);
                return {
                    schemeCode: code,
...
exports.default = HoldingsController;
