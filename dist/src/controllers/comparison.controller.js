...
exports.calculateOverlap = exports.compareFunds = void 0;
...
const overlapSchema = zod_1.z.object({
    fundIds: zod_1.z
        .array(zod_1.z.string())
        .min(2, 'At least 2 fund IDs required')
        .max(5, 'Maximum 5 funds can be compared'),
 ...
 * Jaccard = |A ∩ B| / |A ∪ B|
 */
function calculateJaccardSimilarity(holdings1, holdings2) {
 ...
 */
function calculateWeightedOverlap(holdings1, holdings2) {
    const map1 = new Map(holdings1.map((h) => [h.ticker || h.name, h.percentage]));
    const map2 = new Map(holdings2.map((h) => [h.ticker || h.name, h.percentage]));
 ...
 */
function findCommonHoldings(fundsData) {
    ...
    holdingsMap.forEach((funds, ticker) => {
        if (funds.length >= 2) {
            const avgPercentage = funds.reduce((sum, f) => sum + f.percentage, 0) / funds.length;
            commonHoldings.push({
                ticker,
                name: ticker,
 ...
 */
function calculateSectorOverlap(fundsData) {
    ...
    const sectorComparison = Array.from(allSectors).map((sector) => {
        ...
        const avgPercentage = allocations.reduce((sum, a) => sum + a.percentage, 0) /
            allocations.length;
        const maxDiff = Math.max(...allocations.map((a) => a.percentage)) -
 ...
 */
function calculateReturnCorrelation(fund1Returns, fund2Returns) {
    const periods = [
        'day',
        'week',
        'month',
...
const compareFunds = async (req, res) => {
    try {
        ...
        // Fetch all funds
        const funds = await Promise.all(fundIds.map((id) => getFundModel().findById(id)));
        ...
        for (let i = 0; i < fundsData.length; i++) {
            for (let j = i + 1; j < fundsData.length; j++) {
                ...
                const jaccardSimilarity = calculateJaccardSimilarity(fund1.holdings, fund2.holdings);
                const weightedOverlap = calculateWeightedOverlap(fund1.holdings, fund2.holdings);
                const returnCorrelation = calculateReturnCorrelation(fund1.returns, fund2.returns);
                pairwiseComparisons.push({
                    ...
                    similarity: Number(((jaccardSimilarity + weightedOverlap) / 2).toFixed(2)),
                });
            }
        }
        ...
        const avgJaccardSimilarity = pairwiseComparisons.reduce((sum, p) => sum + p.jaccardSimilarity, 0) /
            pairwiseComparisons.length;
        const avgWeightedOverlap = pairwiseComparisons.reduce((sum, p) => sum + p.weightedOverlap, 0) /
            pairwiseComparisons.length;
        const avgReturnCorrelation = pairwiseComparisons.reduce((sum, p) => sum + p.returnCorrelation, 0) /
            pairwiseComparisons.length;
        const result = {
            funds: fundsSummary,
            pairwiseComparisons,
            ...
            insights: {
                mostSimilarPair: pairwiseComparisons.reduce((max, p) => p.similarity > max.similarity ? p : max),
                leastSimilarPair: pairwiseComparisons.reduce((min, p) => p.similarity < min.similarity ? p : min),
                highestCorrelation: pairwiseComparisons.reduce((max, p) => p.returnCorrelation > max.returnCorrelation ? p : max),
            },
        ...
        return res.json((0, response_1.formatResponse)(result, 'Funds compared successfully'));
    }
    catch (error) {
...
exports.compareFunds = compareFunds;
/**
 * POST /overlap
...
const calculateOverlap = async (req, res) => {
    try {
        ...
        for (let i = 0; i < fundsData.length; i++) {
            for (let j = i + 1; j < fundsData.length; j++) {
                ...
                pairwiseOverlaps.push({
                    ...
                    overlapPercentage: Number(((commonInPair.length /
        ...
        const uniqueHoldings = fundsData.map((fund) => {
            const otherFundsHoldings = new Set(fundsData
                .filter((f) => f.fundId !== fund.fundId)
                .flatMap((f) => f.holdings.map((h) => h.ticker || h.name)));
            const unique = fund.holdings.filter((h) => !otherFundsHoldings.has(h.ticker || h.name));
            return {
                fundId: fund.fundId,
                fundName: fund.name,
                uniqueCount: unique.length,
                uniquePercentage: Number(((unique.length / fund.totalHoldings) * 100).toFixed(2)),
        ...
        const result = {
            ...
            overallMetrics: {
                totalCommonHoldings: commonHoldings.length,
                avgJaccardSimilarity: Number((pairwiseOverlaps.reduce((sum, p) => sum + p.jaccardSimilarity, 0) /
                    pairwiseOverlaps.length).toFixed(2)),
                avgWeightedOverlap: Number((pairwiseOverlaps.reduce((sum, p) => sum + p.weightedOverlap, 0) /
                    pairwiseOverlaps.length).toFixed(2)),
                maxOverlap: Math.max(...pairwiseOverlaps.map((p) => p.weightedOverlap)),
                minOverlap: Math.min(...pairwiseOverlaps.map((p) => p.weightedOverlap)),
            },
            insights: {
                mostOverlappingPair: pairwiseOverlaps.reduce((max, p) => p.weightedOverlap > max.weightedOverlap ? p : max),
                leastOverlappingPair: pairwiseOverlaps.reduce((min, p) => p.weightedOverlap < min.weightedOverlap ? p : min),
                mostDiverseFund: uniqueHoldings.reduce((max, f) => f.uniquePercentage > max.uniquePercentage ? f : max),
            },
        ...
        return res.json((0, response_1.formatResponse)(result, 'Overlap analysis completed successfully'));
    }
    catch (error) {
        if (error instanceof zod_1.z.ZodError) {
            console.error('❌ Validation error:', error.errors);
