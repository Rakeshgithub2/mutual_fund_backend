...
exports.getFundBySchemeCode = getFundBySchemeCode;
exports.checkDataSourceHealth = checkDataSourceHealth;
const mongodb_serverless_1 = require("../lib/mongodb-serverless");
const oracle_mfapi_fallback_service_1 = require("../services/oracle-mfapi-fallback.service");
...
async function searchFunds(req, res) {
    try {
        ...
        if (minNav) {
            filter['nav.value'] = { $gte: parseFloat(minNav) };
        }
        ...
        console.log(`⚡ DB results insufficient (${funds.length}), querying Oracle/MFAPI...`);
        try {
            const realtimeResults = await oracle_mfapi_fallback_service_1.oracleFallbackService.searchFundRealtime(searchQuery);
        ...
        if (funds.length < Math.min(3, limitNum)) {
            ...
            await (0, queue_service_1.addToMissingFundsQueue)(searchQuery);
        }
 ...
 */
async function searchFundsRegex(req, res) {
    ...
    res.json({
        ...
        count: funds.length,
        warning: 'Using slow regex search. Create text index for better performance.',
    });
}
/**
...
async function getFundBySchemeCode(req, res) {
    try {
        ...
        const realtimeFund = await oracle_mfapi_fallback_service_1.oracleFallbackService.fetchBySchemeCode(schemeCode);
        if (realtimeFund) {
            res.json({
                success: true,
                data: realtimeFund,
 ...
 */
async function checkDataSourceHealth(req, res) {
    try {
        const health = await oracle_mfapi_fallback_service_1.oracleFallbackService.healthCheck();
        const db = await (0, mongodb_serverless_1.getDb)();
        const collection = db.collection('fund_master');
        const dbCount = await collection.countDocuments();
        // Get queue stats
        const queueStats = await (0, queue_service_1.getQueueStats)();
        res.json({
            success: true,
            database: {
                connected: true,
