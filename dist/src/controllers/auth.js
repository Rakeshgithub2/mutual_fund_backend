...
exports.refreshTokens = exports.login = exports.register = void 0;
...
const register = async (req, res) => {
    try {
        ...
        const usersCollection = mongodb_1.mongodb.getCollection('users');
        ...
        if (existingUser) {
            ...
            if (existingUser.provider === 'google') {
                res.status(409).json({
                    error: 'This email is registered using Google. Please login with Google.',
                });
                return;
            }
            res.status(409).json({
        ...
        // Store refresh token
        const refreshTokensCollection = mongodb_1.mongodb.getCollection('refresh_tokens');
...
const login = async (req, res) => {
    try {
        ...
        res.json((0, response_1.formatResponse)({
            user: {
                id: userId,
                email: user.email,
                name: user.name,
...
const refreshTokens = async (req, res) => {
    try {
        ...
        const newRefreshToken = (0, auth_1.generateRefreshToken)({ id: userId });
        // Update refresh token
        await refreshTokensCollection.updateOne({ _id: storedToken._id }, {
            $set: {
