...
exports.refreshRankings = exports.getAllCategoryLeaders = exports.getRollingReturnRankings = exports.getRiskAdjustedRankings = exports.getSubCategoryLeaders = exports.getCategoryLeaders = exports.getTopFunds = void 0;
 ...
 */
function transformForMobile(funds, includeDetails = false) {
...
const getTopFunds = async (req, res) => {
    try {
        ...
        if (req.query.schemeType) {
            criteria.schemeType = req.query.schemeType;
        }
        if (req.query.minAUM) {
            criteria.minAUM = parseInt(req.query.minAUM);
        }
        const rankedFunds = await ranking_service_1.rankingService.getTopFunds(limit, criteria);
...
exports.getTopFunds = getTopFunds;
/**
...
const getCategoryLeaders = async (req, res) => {
    try {
        ...
        const leaders = await ranking_service_1.rankingService.getCategoryLeaders(category, limit);
...
exports.getCategoryLeaders = getCategoryLeaders;
/**
 * GET /api/rankings/subcategory/:category/:subcategory
 * Get sub-category leaders
 */
const getSubCategoryLeaders = async (req, res) => {
    try {
        const { category, subcategory } = req.params;
        const limit = parseInt(req.query.limit) || 10;
        const includeDetails = req.query.details === 'true';
        const leaders = await ranking_service_1.rankingService.getSubCategoryLeaders(category, subcategory, limit);
        const mobileData = transformForMobile(leaders, includeDetails);
        return res.json({
            success: true,
...
exports.getSubCategoryLeaders = getSubCategoryLeaders;
/**
 * GET /api/rankings/risk-adjusted
 * Get risk-adjusted rankings (Sharpe/Sortino based)
 */
const getRiskAdjustedRankings = async (req, res) => {
    try {
        ...
        const rankedFunds = await ranking_service_1.rankingService.getRiskAdjustedRankings(limit, criteria);
        const mobileData = transformForMobile(rankedFunds, includeDetails);
        return res.json({
            success: true,
            ...
            metadata: {
                count: mobileData.length,
                criteria,
                methodology: 'Risk-adjusted scoring: 60% Sharpe ratio, 40% Sortino ratio',
                calculatedAt: new Date(),
                expandable: !includeDetails,
            },
        });
...
exports.getRiskAdjustedRankings = getRiskAdjustedRankings;
/**
 * GET /api/rankings/rolling/:period
 * Get rolling return rankings (2Y, 3Y, 5Y)
 */
const getRollingReturnRankings = async (req, res) => {
    try {
        ...
        const criteria = {
            returnPeriod: period.toLowerCase(),
            limit,
        };
        if (req.query.category) {
            criteria.category = req.query.category;
        }
        const rankedFunds = await ranking_service_1.rankingService.getRollingReturnRankings(period.toLowerCase(), limit, criteria);
        const mobileData = transformForMobile(rankedFunds, includeDetails);
        return res.json({
            success: true,
...
exports.getRollingReturnRankings = getRollingReturnRankings;
/**
 * GET /api/rankings/all-categories
 * Get leaders from all categories (mobile dashboard view)
 */
const getAllCategoryLeaders = async (req, res) => {
    try {
        const perCategory = parseInt(req.query.limit) || 5;
        const includeDetails = req.query.details === 'true';
        const categories = [
            'equity',
            'debt',
        ...
        for (const category of categories) {
            const leaders = await ranking_service_1.rankingService.getCategoryLeaders(category, perCategory);
        ...
        return res.json({
            ...
            metadata: {
                ...
                perCategory,
                totalFunds: Object.values(allLeaders).reduce((sum, arr) => sum + arr.length, 0),
...
exports.getAllCategoryLeaders = getAllCategoryLeaders;
/**
 * POST /api/rankings/refresh
 * Clear ranking cache (admin endpoint)
 */
const refreshRankings = async (req, res) => {
    try {
        ranking_service_1.rankingService.clearCache();
        return res.json({
            success: true,
            message: 'Ranking cache cleared successfully. New rankings will be calculated on next request.',
        });
    }
    catch (error) {
...
exports.refreshRankings = refreshRankings;
