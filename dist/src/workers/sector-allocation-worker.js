...
const sectorAllocation_service_1 = require("../services/sectorAllocation.service");
...
async function main() {
    try {
        ...
        const stats = await sectorAllocation_service_1.SectorAllocationService.getStats();
        ...
        // Process funds
        console.log(`🔄 Processing ${Math.min(limit, stats.fundsWithoutSectors)} funds...\n`);
        await sectorAllocation_service_1.SectorAllocationService.processAllFunds(limit);
        // Show updated statistics
        console.log('\n📊 Updated Statistics:');
        const updatedStats = await sectorAllocation_service_1.SectorAllocationService.getStats();
        console.log(`   Total Equity Funds: ${updatedStats.totalEquityFunds}`);
        console.log(`   ✅ Funds with Sectors: ${updatedStats.fundsWithSectors}`);
        console.log(`   ❌ Funds without Sectors: ${updatedStats.fundsWithoutSectors}`);
        console.log(`   📈 Coverage: ${updatedStats.coveragePercent}%`);
        console.log(`   🎯 Improvement: +${(updatedStats.coveragePercent - stats.coveragePercent).toFixed(2)}%\n`);
