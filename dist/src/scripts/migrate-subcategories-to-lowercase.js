    ...
    try {
        ...
        for (const fund of allFunds) {
            if (fund.subCategory) {
                ...
                const newSubCategory = subcategoryMappings[oldSubCategory] ||
                    oldSubCategory.toLowerCase().replace(/[\s_\-&\/]/g, '');
                if (oldSubCategory !== newSubCategory) {
                    await collection.updateOne({ _id: fund._id }, { $set: { subCategory: newSubCategory } });
                    console.log(`  ✅ Updated: "${oldSubCategory}" → "${newSubCategory}"`);
                    updatedCount++;
                }
                else {
                    skippedCount++;
        ...
        console.log(`   Skipped: ${skippedCount} funds (already in correct format)`);
        // Verify the results
