...
async function main() {
    ...
    try {
        ...
        await (0, schemas_1.createIndexes)(db);
        ...
        if (process.env.ALPHA_VANTAGE_KEY) {
            ...
            console.log('⚠️  Alpha Vantage API key not configured - skipping historical data');
        ...
        stats.topFundsByAUM.forEach((fund, i) => {
            ...
            console.log(`     House: ${fund.fundHouse}, AUM: ₹${(fund.aum / 10000000).toFixed(2)}Cr`);
        });
        console.log('\n═'.repeat(60));
        ...
        const checks = [
            ...
            },
            {
                ...
                passed: stats.bySource.amfi > 0 &&
                    (stats.bySource.nse > 0 || stats.bySource.yahoo > 0),
            },
        ];
        checks.forEach((check) => {
            const icon = check.passed ? '✅' : '❌';
            console.log(`${icon} ${check.name}: ${check.actual} (Required: ${check.required})`);
        });
        ...
        if (allPassed) {
            ...
            console.log('⚠️  Some acceptance criteria not met - may need additional data sources');
        }
        console.log('═'.repeat(60));
        console.log('\n📝 NEXT STEPS:');
