...
function generateAdditionalEquityFunds() {
...
function generateAdditionalCommodityFunds() {
    const additionalFunds = [];
        ...
        for (const fund of allFunds) {
            try {
                ...
                const fundData = {
                    ...
                    launchDate: new Date(Date.now() - Math.random() * 5 * 365 * 24 * 60 * 60 * 1000), // Random date within 5 years
                    aum: fund.aum,
                    expenseRatio: fund.expenseRatio,
                    exitLoad: Math.random() < 0.3 ? Math.random() * 2 : 0, // 30% chance of exit load
                    minInvestment: fund.fundType === 'etf' ? 1 : Math.random() < 0.5 ? 500 : 1000,
                    sipMinAmount: fund.fundType === 'etf' ? 0 : Math.random() < 0.5 ? 500 : 1000,
...
function generateSearchTerms(fund) {
    return [
...
exports.default = importComprehensiveFunds;
