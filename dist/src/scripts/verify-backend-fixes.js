...
async function testFundPagination() {
    ...
    try {
        ...
        const response = await axios_1.default.get(`${API_URL}/api/funds`, {
        ...
        if (pagination.total < 100) {
            log(COLORS.red, `❌ FAIL: Only ${pagination.total} funds found (expected 4000+)`);
            return false;
        }
        if (data.length !== 500) {
...
async function testMarketIndices() {
    ...
    try {
        const start = Date.now();
        const response = await axios_1.default.get(`${API_URL}/api/market/summary`, {
            timeout: 10000,
        });
        const elapsed = Date.now() - start;
        ...
        const isStatic = data.every((idx) => staticValues.includes(idx.value));
        if (isStatic) {
            log(COLORS.red, '❌ FAIL: Market data appears to be STATIC (default values)');
        ...
        data.forEach((idx) => {
            ...
            log(changeColor, `   ${idx.index}: ${idx.value.toFixed(2)} ${changeSymbol} ${idx.changePercent.toFixed(2)}% (${idx.dataSource || 'unknown'})`);
        });
...
async function testSectorAllocation() {
    ...
    try {
        ...
        const fundsResponse = await axios_1.default.get(`${API_URL}/api/funds`, {
        ...
        for (const fund of funds) {
            try {
                const detailsResponse = await axios_1.default.get(`${API_URL}/api/funds/${fund.fundId}/details`, { timeout: 5000 });
                const fundDetails = detailsResponse.data.data;
                if (fundDetails.sectorAllocation &&
                    fundDetails.sectorAllocation.length > 0) {
                    withSectors++;
                    log(COLORS.green, `   ✅ ${fund.name}: ${fundDetails.sectorAllocation.length} sectors`);
                }
                else {
                    withoutSectors++;
...
async function testPerformance() {
    ...
    for (const test of tests) {
        const start = Date.now();
        try {
            await axios_1.default.get(`${API_URL}${test.url}`, {
            ...
            if (elapsed <= test.maxTime) {
                ...
                log(COLORS.yellow, `   ⚠️  ${test.name}: ${elapsed}ms (slow, expected <${test.maxTime}ms)`);
            }
        }
        catch (error) {
...
async function main() {
    ...
    log(COLORS.blue, '╔════════════════════════════════════════════════════════════╗');
    log(COLORS.blue, '║                                                            ║');
    log(COLORS.blue, '║         BACKEND VERIFICATION & FIX VALIDATION              ║');
    log(COLORS.blue, '║                                                            ║');
    log(COLORS.blue, '╚════════════════════════════════════════════════════════════╝');
    log(COLORS.blue, `\n🔗 Testing API: ${API_URL}\n`);
    // Run all tests
