...
async function main() {
    ...
    try {
        ...
        console.log('⚠️  Yahoo Finance API requires valid subscription - skipping for now\n');
        // Generate summary
        console.log('═'.repeat(60));
        ...
        stats.byFundHouse.forEach((house, i) => {
            console.log(`  ${i + 1}. ${house._id}: ${house.count} funds, ₹${(house.totalAum / 10000000).toFixed(2)}Cr`);
        });
        console.log('\nTop 10 Funds by AUM:');
        stats.topFundsByAUM.forEach((fund, i) => {
            console.log(`  ${i + 1}. ${fund.name.substring(0, 60)}`);
            console.log(`     ${fund.fundHouse} | ${fund.category} | ₹${(fund.aum / 10000000).toFixed(2)}Cr`);
        });
