...
function validateConfig() {
    ...
    if (!exports.config.google.clientId) {
        errors.push('GOOGLE_CLIENT_ID is required');
    }
    if (exports.config.server.nodeEnv === 'production') {
        if (exports.config.jwt.accessTokenSecret.includes('change-in-production')) {
            errors.push('JWT_ACCESS_SECRET must be set in production');
        }
        if (exports.config.jwt.refreshTokenSecret.includes('change-in-production')) {
...
function logConfig() {
    ...
    console.log(`   - Environment: ${exports.config.server.nodeEnv}`);
    console.log(`   - Port: ${exports.config.server.port}`);
    console.log(`   - MongoDB: ${exports.config.mongodb.uri.replace(/\/\/.*@/, '//***@')}`);
    console.log(`   - Redis: ${exports.config.redis.host}:${exports.config.redis.port}`);
    console.log(`   - Google OAuth: ${exports.config.google.clientId ? 'Configured' : 'Not configured'}`);
    console.log(`   - Yahoo Finance API: ${exports.config.apis.yahooFinance.apiKey ? 'Configured' : 'Not configured'}`);
    console.log(`   - Alpha Vantage API: ${exports.config.apis.alphaVantage.apiKey ? 'Configured' : 'Not configured'}`);
    console.log(`   - Atlas Search: ${exports.config.features.enableAtlasSearch ? 'Enabled' : 'Disabled'}`);
    console.log(`   - Background Jobs: ${exports.config.features.enableBackgroundJobs ? 'Enabled' : 'Disabled'}\n`);
}
