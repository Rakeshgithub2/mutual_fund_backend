...
async function compareFunds(req, res) {
    try {
        ...
        const metrics = {
            bestOneYearReturn: Math.max(...comparison.map((f) => f.returns.oneYear || 0)),
            bestThreeYearReturn: Math.max(...comparison.map((f) => f.returns.threeYear || 0)),
            bestFiveYearReturn: Math.max(...comparison.map((f) => f.returns.fiveYear || 0)),
            lowestExpenseRatio: Math.min(...comparison.map((f) => f.expenseRatio).filter((e) => e > 0)),
        ...
        const rankings = comparison
            ...
            if (fund.returns.oneYear === metrics.bestOneYearReturn)
                score += 15;
            if (fund.returns.threeYear === metrics.bestThreeYearReturn)
                score += 15;
            if (fund.returns.fiveYear === metrics.bestFiveYearReturn)
                score += 10;
            // Expense ratio score (30% weight) - lower is better
            if (fund.expenseRatio === metrics.lowestExpenseRatio)
                score += 30;
            // AUM score (20% weight)
            if (fund.aum === metrics.highestAUM)
                score += 20;
        ...
        res.json({
            ...
            data: {
                ...
                summary: {
                    ...
                    averageExpenseRatio: (comparison.reduce((sum, f) => sum + f.expenseRatio, 0) /
                        comparison.length).toFixed(2),
                    averageOneYearReturn: (comparison.reduce((sum, f) => sum + (f.returns.oneYear || 0), 0) /
                        comparison.length).toFixed(2),
                },
            },
        });
