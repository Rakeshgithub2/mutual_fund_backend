...
export class CalculationService {
   ...
   * FV = P × ((1 + r)^n - 1) / r × (1 + r)
   */
  calculateSIP(monthlyInvestment: number, rateOfReturn: number, years: number) {
   ...
   * FV = P × (1 + r)^n
   */
  calculateLumpsum(principal: number, rateOfReturn: number, years: number) {
   ...
   */
  calculateCAGR(beginningValue: number, endingValue: number, years: number) {
   ...
   */
  calculateLTCG(gainAmount: number) {
   ...
   */
  calculateSTCG(gainAmount: number) {
   ...
   */
  calculateEmergencyFund(monthlyExpenses: number, months: number = 6) {
  ...
  calculateGoalPlanning(
    ...
    const lumpSumNeeded =
      additionalNeeded / Math.pow(1 + expectedReturn / 100, yearsToGoal);
   ...
   */
  calculateNAVReturns(purchaseNAV: number, currentNAV: number, units: number) {
  ...
  calculateStepUpSIP(
    ...
    return {
      ...
      returns: Math.round(returns),
      percentageGain: ((returns / totalInvested) * 100).toFixed(2),
      finalMonthlyAmount: Math.round(
        monthlyAmount / (1 + annualIncrement / 100)
      ),
    };
  }
   ...
   */
  parseCalculationQuery(query: string): { type: string; params: any } | null {
    const queryLower = query.toLowerCase();

    // SIP calculation
    if (
      queryLower.includes('sip') ||
