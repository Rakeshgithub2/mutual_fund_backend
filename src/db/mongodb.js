...
class MongoDB {
    ...
    async doConnect() {
        ...
        try {
            ...
            console.log('📍 [MONGO] Target:', DATABASE_URL.replace(/:[^:@]+@/, ':***@'));
            // Create client with production-optimized settings
            this.client = new mongodb_1.MongoClient(DATABASE_URL, MONGO_OPTIONS);
     ...
     */
    async queryWithTiming(collectionName, operation, queryFn) {
        const startTime = Date.now();
        try {
            ...
            if (queryTime > 1000) {
                console.warn(`⚠️  [MONGO] Slow query: ${collectionName}.${operation} took ${queryTime}ms`);
            }
            else {
                console.log(`⚡ [MONGO] ${collectionName}.${operation} = ${queryTime}ms`);
            }
            return result;
        }
        catch (error) {
            const queryTime = Date.now() - startTime;
            console.error(`❌ [MONGO] Query failed: ${collectionName}.${operation} after ${queryTime}ms`, error);
            throw error;
        }
    }
    async disconnect() {
