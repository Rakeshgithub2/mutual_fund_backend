...
Use realistic data for Indian equity mutual funds. Include actual Indian companies like Reliance, HDFC Bank, Infosys, TCS, ICICI Bank, Bharti Airtel, ITC, Kotak Bank, L&T, Axis Bank, etc.`;
    ...
    try {
      ...
      if (geminiResult?.holdings?.length > 0) {
        ...
        await db.collection('sector_allocation').updateOne(
          { schemeCode: fundCode },
          {
            $set: {
              schemeCode: fundCode,
        ...
        console.log(
          `   ✅ Updated: ${geminiResult.holdings.length} holdings, ${Object.keys(geminiResult.sectors || {}).length} sectors`
        );
        successCount++;
      } else {
  ...
  sampleHoldings.forEach((h) => {
    ...
    console.log(
      ...
      h.holdings
        ?.slice(0, 3)
        .map((x: any) => `${x.company} (${x.holdingPercent}%)`)
        .join(', ')
    );
  });
