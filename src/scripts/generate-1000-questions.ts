...
async function generateQuestionDetails(
  question: string,
  category: string,
  index: number
) {
...
Make it simple for first-time investors. Use Indian context (₹, SEBI, Indian tax rules 2026).`;
  ...
  return {
    ...
    keywords: generateKeywords(question),
    definition:
      'A financial concept related to investing and wealth management.',
    points: [
...
function determineLevel(
  ...
  if (beginnerKeywords.some((k) => q.includes(k))) return 'beginner';
  if (advancedKeywords.some((k) => q.includes(k))) return 'advanced';
  return 'intermediate';
}

// Main generation function
async function generateAllQuestions() {
  console.log('🚀 Starting generation of 1000 knowledge base entries...\n');
  ...
  for (const [categoryKey, questions] of Object.entries(questionsToGenerate)) {
    console.log(
      `\n📁 Processing category: ${categoryKey} (${questions.length} questions)`
    );

    const categoryName =
      categoryKey === 'mutualFunds'
        ? 'Mutual Funds'
        : categoryKey === 'stocks'
          ? 'Stocks'
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      console.log(
        `  Processing ${currentId}/1000: ${question.substring(0, 50)}...`
      );

      const entry = await generateQuestionDetails(
        question,
        categoryName,
        currentId
      );
      ...
      if (currentId % 10 === 0) {
        console.log(
          `    Processed ${currentId} questions, taking a short break...`
        );
  ...
  const outputPath = path.join(
    __dirname,
    '../data/knowledge-base-1000-generated.ts'
  );
...
`;
  ...
  console.log(
    `\n✅ Successfully generated ${allEntries.length} knowledge base entries!`
  );
  console.log(`📁 Saved to: ${outputPath}`);
  console.log(`📊 Breakdown:`);

  const breakdown = allEntries.reduce((acc: any, entry: any) => {
