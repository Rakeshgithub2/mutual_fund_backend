 ...
 */
function getTemplateKey(category?: string, subCategory?: string): string {
  ...
  if (
    ...
    return 'Debt';
  if (cat === 'hybrid' || cat.includes('hybrid')) return 'Hybrid';
  if (cat === 'equity' || cat.includes('equity')) return 'Flexi Cap'; // Default for generic equity
 ...
 */
function addVariation(holdings: any[], fundName: string): any[] {
  ...
  return holdings.map((h, index) => {
    ...
    const variation = ((seed + index * 17) % 100) / 100 - 0.5;
    const newWeight = Math.max(0.1, h.weight + variation);
 ...
 */
export function getFallbackHoldings(
  schemeCode: string,
  fundName: string,
  category?: string,
  subCategory?: string
  ...
  const templateKey = getTemplateKey(category, subCategory);
  const template =
    HOLDINGS_TEMPLATES[templateKey] || HOLDINGS_TEMPLATES['Default'];

  // Add variation to make each fund slightly different
  const holdings = addVariation(template.holdings, fundName || schemeCode);
