...
export function normalizeExternalFund(mfapiData: any) {
  if (!mfapiData || !mfapiData.meta) return null;
  return {
    fundId:
      mfapiData.meta.scheme_code ||
...
export function normalizeExternalHoldings(mfapiData: any, schemeCode: string) {
  const holdings = (mfapiData.data?.[0]?.holdings || []).map((h: any) => ({
