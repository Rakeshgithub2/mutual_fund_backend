...
router.post('/', optionalAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (isCalculationQuery(userQuery)) {
      const calcParams = parseCalculationQuery(userQuery);
      if (calcParams) {
    ...
    const questions = extractMultipleQuestions(userQuery);
    ...
    if (questions.length > 1) {
      ...
      const matches = await findMultipleMatches(questions);
      ...
      if (matches.length >= 2) {
        ...
        if ((req as any).user && mongodb.isConnected()) {
    ...
    if (knowledgeMatch && score >= 0.4) {
      ...
      const response = formatKnowledgeResponse(knowledgeMatch);
    ...
    const relevantKnowledge = await searchQuestions(userQuery, 3);
...
router.get('/knowledge', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (search && typeof search === 'string') {
      ...
      results = results.filter(
        (k) =>
          k.question.toLowerCase().includes(searchLower) ||
    ...
    let grouped: Record<string, typeof results> | null = null;
    if (!category) {
      grouped = {};
...
router.get('/knowledge/:id', async (req: Request, res: Response) => {
  ...
  const related = entry.relatedQuestions
    ? knowledgeBase.filter((k) => entry.relatedQuestions?.includes(k.id))
    : [];
...
router.post('/calculate', async (req: Request, res: Response) => {
  ...
  try {
    const result = performCalculation(type, params);
